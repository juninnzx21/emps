...
class EMPS_DB
{
    ...
    public function connect()
    {
        ...
        $string = "host={$emps_db_config['host']} dbname={$emps_db_config['database']} user={$emps_db_config['user']} password={$emps_db_config['password']} options='--client_encoding=UTF8'";
        $this->db = pg_connect($string);

        if (!$this->db) {
            error_log("postgres error: " . pg_last_error());
            return;
        }
...
//        error_log($table." > ".$where);
    ...
    {
        return pg_escape_string($this->db, $txt);
    }
